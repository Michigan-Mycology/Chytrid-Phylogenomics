...
#### First Split of Original Ploidy Figure (AF + kmer + scatter) ####
...
dipl_af_pane = ggplot(cali_af) +
  geom_density(aes(x=p), fill="blue", alpha=0.4, color="blue", size=1) +
  scale_x_continuous(expand = c(0,0), limits = c(0,1)) +
...
des = "1255
3455"
panel = hapl_kmer_pane + dipl_kmer_pane + hapl_af_pane + dipl_af_pane + ploidy_scatter + plot_layout(design = des)

ggsave(filename = "~/dev/Chytrid-Phylogenomics/figures_pnas_revisions/3_ploidy/3a_ploidy.pdf",
       plot = panel,
       device = cairo_pdf,
       width = 8.5,
       height = 5.5,
...
#ggsave("~/dev/Chytrid-Phylogenomics/figures_pnas_revisions/3_ploidy/marginal_tree_pane_nodenum.pdf",
#       plot = marginal_tree_pane,
#       width = 8.5,
#       height = 11,
#       units = "in",
#       device = cairo_pdf)
...
2
"
fig = marginal_tree_pane +
  combined_anc_bars_plot +
  plot_layout(design=patchwork_design)

fig
ggsave("~/dev/Chytrid-Phylogenomics/figures_pnas_revisions/3_ploidy/ploidy_3b.pdf",
       plot = fig,
       width = 8.5,
       height = 5.5,
       device = "pdf")

### Combine anc state bars from marginal and mcmc ASR as PIES
combined_anc_bars = rbind(marginal_anc_bars, mcmc_anc_bars) %>%
  spread(key, value) %>%
  mutate(group = factor(group, levels=c("Fungi", "Blastocladiomycota", "Chytridiomycota",
                                        "Monoblepharomycota", "Neocallimastigomycota",
...
plts = list()
i=1
for (t in levels(combined_anc_bars$taxon)) {
  this_data = combined_anc_bars %>% filter(taxon == t)
  plts[[i]] = ggplot(this_data, aes(x= group, y=value, group = type, fill = key)) +
    geom_bar(data = subset(this_data, type=="marginal"),
             aes(x= group, y=value, group = type, fill = key),
             stat="identity", color="black",position="stack", width=barwidth) +
    geom_text(data = subset(this_data, type=="marginal"),
             aes(label = paste(format(round(value*100, 2), nsmall = 2), "%", sep=""), x = group),
             position = position_stack(vjust = 0.5), size = 1.7) +
    coord_polar("y", start=0) +
    scale_fill_manual(values=c("red", "blue")) +
...
for (t in levels(combined_anc_bars$taxon)) {
  this_data = combined_anc_bars %>% filter(taxon == t)
  plts[[i]] = ggplot(this_data, aes(x= group, y=value, group = type, fill = key)) +
    geom_bar(data = subset(this_data, type=="mcmc"),
             aes(x= group, y=value, group = type, fill = key),
             stat="identity", color="black",position="stack", width=barwidth, alpha = 0.7) +
    geom_text(data = subset(this_data, type=="mcmc"),
              aes(label = paste(format(round(value*100, 2), nsmall = 2), "%", sep=""), x = group),
              position = position_stack(vjust = 0.5), size = 1.7) +
    coord_polar("y", start=0) +
    scale_fill_manual(values=c("red", "blue")) +
...
all_pies = plts[[1]] + plts[[2]] + plts[[3]] + plts[[4]] + plts[[5]] + plts[[6]] + plts[[7]] + plts[[8]] +
  plts[[9]] + plts[[10]] + plts[[11]] + plts[[12]] + plts[[13]] + plts[[14]] + plts[[15]] + plts[[16]]
ggsave("~/dev/Chytrid-Phylogenomics/figures_pnas_revisions/3_ploidy/ploidy_3b_pies.pdf",
       plot = all_pies,
       width = 8.5,
       height = 5.5,
       device = "pdf")
