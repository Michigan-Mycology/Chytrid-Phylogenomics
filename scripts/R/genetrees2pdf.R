...
setwd("/home/aimzez/DATA/phylogeny/round7_mancur_of_round5_spikein_rm_poly/pdf")
...
for (f in files) {
  ...
  annotated_tree = annotated_tree +
    ggplot2::xlim(0, x_max*1.5)
  #break
  ggsave(filename = paste(f,".round7.probtaxa_HL", ".pdf", sep=""), plot = annotated_tree, device = "pdf", width=17, height=(14*((ntips)/140)), units="in", limitsize = FALSE)
 }
annotated_tree
