...
tree_scores = read_delim("~/DATA/phylogeny/hit_report_all.csv", delim="\t", col_names=c("protein", "marker", "evalue", "score")) %>%
...
for (t in tims_markers) {
  ...
  these_tree_scores = tree_scores %>%
    filter(marker == t) %>%
    filter(tiplab %in% tree$tip.label)
  the_plot = ggplot(these_tree_scores, aes(x=score)) +
