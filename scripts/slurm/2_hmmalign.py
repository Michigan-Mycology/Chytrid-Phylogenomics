...
for idx, chunk in enumerate(todo_spl):
    ...
    for line in chunk:
        ...
        sg.add_command("perl -p -e 'if (! /^>/) {{ s/[ZBzbXx\*]/-/g }}' {} > {}".format(
            line.replace('.fasta', '.esltmp'), line.replace('.fasta', '.clnaln')))
        # sg.add_command(f"rm {line.replace('.fasta','.esltmp')}")
        # sg.add_command(f"trimal -automated1 -in
        # {line.replace('.fasta','.clnaln')} -out
        # {line.replace('.fasta','.aa.filter')}")
