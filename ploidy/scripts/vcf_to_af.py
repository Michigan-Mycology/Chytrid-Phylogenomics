...
parser.add_argument("-c", "--select_contigs", action="store", required=False, default = None, metavar="str", help="A list of contig headers to keep. SNPs on the rest will not be output.")
parser.add_argument("--skip_na_mqrs", action="store_true", required=False, help="Skips contigs that don't have a value in the MQRankSum field.")
parser.add_argument("--dpfilt", action="store_true", help="Filter SNPs by depth so that only SNPs with depths >= Q1 and <= Q3 are included.")
parser.add_argument("--mqrsfilt", action="store_true", help="Filter SNPs by MQRankSum. Values outside 1 standard deviation from the mean will be excluded.")
parser.add_argument("-s", "--strain", action="store", required=True, help="Strain name to display at the top of the final plots.")
parser.add_argument("-p", "--plot", action="store_true", help="Print an allele frequency histogram to PDF using R.")
parser.add_argument("--kmercounts", action="store", required=False, help="Optionally provide the path to and prefix of khist and peaks files from kmercountexact for incorporation into plots.")
...
if args.plot:
    ...
    p = subprocess.Popen(plotcmd, stdout = subprocess.PIPE, stderr = subprocess.PIPE)
